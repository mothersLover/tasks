...
public class KindergartenAdventuresSegmentTree {
    ...
    @Test
    public void test1() {
        int[] minutes = new int[] {1, 0, 0};
        SegmentTree segmentTree = new SegmentTree(minutes);
        int bestIndex = segmentTree.getBestIndex();
        Assert.assertEquals(2, bestIndex);
    }

    @Test
    public void test2() {
        int[] minutes = new int[] {0, 1, 2};
        SegmentTree segmentTree = new SegmentTree(minutes);
        int bestIndex = segmentTree.getBestIndex();
        Assert.assertEquals(1, bestIndex);
    }

    @Test
    public void test3() {
        int[] minutes = new int[] {3, 0, 1, 2};
        SegmentTree segmentTree = new SegmentTree(minutes);
        int bestIndex = segmentTree.getBestIndex();
        Assert.assertEquals(2, bestIndex);
    }
    ...
    public static class SegmentTree {
        ...
        private void build(int[] arr) {
            for (int i = 0; i < length; i++) {
                ...
                if (start <= end) {
                    modify(start, end - start + 1, 1);
                } else {
                    modify(start, length - start, 1);
                    modify(0, end + 1, 1);
                }
            }
        }
